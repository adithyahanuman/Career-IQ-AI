...
(function () {
    ...
    function scoreColor(s) {
        if (s >= 80) return '#22d3a5';
        if (s >= 65) return '#4f8ef7';
    ...
    function drawTrendChart(currentScore) {
        ...
        const gridColor = isLight ? 'rgba(14,100,180,0.08)' : 'rgba(255,255,255,0.05)';
    ...
    function populateRoadmap(data) {
        ...
        const PROJECT_COLORS = ['#6366f1','#8b5cf6','#0ea5e9','#f59e0b','#10b981','#f43f5e'];
    ...
    function populatePlacementIQ(pgResume) {
        ...
        if (atsScore) {
            ...
            atsScore.style.color = atsRisk === 'low' ? '#22d3a5' : atsRisk === 'medium' ? '#fbbf24' : '#f87171';
        }
        if (atsSub) {
        ...
        buildSkillBars(techSkills, 'piqTechSkills', '#6366f1');
        buildSkillBars(softSkills, 'piqSoftSkills', '#22d3a5');
